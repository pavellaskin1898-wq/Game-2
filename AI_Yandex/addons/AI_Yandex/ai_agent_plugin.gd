...
var dock_container: Control = null
...
"""
	...
	# Добавляем панель в правую часть редактора как док
	...
	if right_dock:
		...
		dock_container.add_child(main_panel)
	else:
		# Если не нашли, добавляем прямо в базовый контроль
		dock_control.add_child(main_panel)
		dock_container = dock_control
